...
try{
  ...
  $splitSecond = explode("Downey Long Course", $text);
}
...
function printEvent($girlsEventNumber, $girlsEligibility, $girlsMin, $eventName, $eventAge, $boysMin, $boysEligibility, $boysEventNumber, $meetDate, $warmUpTime1, $meetStartTime1)
{
	...
	$conn = $GLOBALS['conn'];
	if($girlsEligibility == "Y" && $boysEligibility=="Y")
	{
		$sql = "insert into event values($girlsEventNumber, $meetId, '".$eventName."', 'Mixed','".$minAge."', '".$maxAge."', '".$meetDate."', '".$girlsMin."', '".$warmUpTime."', '".$meetStartTime."','Morning','N/A')";
		UpdateDB($conn, $sql);
	}
	else if($girlsEligibility == "Y")
	{
		$sql = "insert into event values($girlsEventNumber, $meetId, '".$eventName."', 'Girls','".$minAge."', '".$maxAge."', '".$meetDate."', '".$girlsMin."', '".$warmUpTime."', '".$meetStartTime."','Morning','N/A')";
		UpdateDB($conn, $sql); 
	}
	else //if($boysEligibility == "Y")
	{
        $sql = "insert into event values('".$boysEventNumber."', '".$meetId."', '".$eventName."', 'Boys','".$minAge."', '".$maxAge."', '".$meetDate."', '".$boysMin."', '".$warmUpTime."', '".$meetStartTime."','Morning','N/A')";
	    UpdateDB($conn, $sql); 
	}
